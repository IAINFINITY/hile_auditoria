...
export function LogsView({
  ...
  const runsPerPage = 4;
  const pages = Math.max(1, Math.ceil(runsByDate.length / runsPerPage));
  ...
  const visibleRunsByDate = useMemo(() => {
    const start = (safePage - 1) * runsPerPage;
    return runsByDate.slice(start, start + runsPerPage);
  }, [runsByDate, safePage]);
      ...
      <div className="section-inner">
        ...
        >
          <div className="hile-section-stack">
            <div id="logs-saude" style={{ scrollMarginTop: "96px" }}>
              <HileCardGrid cols={3}>
                <HileKpiCard
                  label="Chatwoot"
              ...
              <HileSurfaceCard title="Saúde rápida" description="Leitura resumida dos pontos mais sensíveis do ambiente." tone="soft">
                <div className="hile-section-stack">
                  <HileInlineInsight title="Status geral">
                    <span className="hile-status-summary" title={statusTooltip}>
                      {statusSummary}
                    </span>
                  </HileInlineInsight>
            ...
            <div id="logs-execucao" style={{ scrollMarginTop: "96px" }}>
              <HileSurfaceCard
              ...
              >
                {!isRunningOverview ? (
                  ...
                  <div className="hile-section-stack">
                    ...
                    <div className="logs-list logs-list-scroll" style={{ marginTop: 10 }}>
            ...
            <div id="logs-recentes" style={{ scrollMarginTop: "96px" }}>
              <HileSurfaceCard
                title="Execuções recentes"
                description="Histórico agrupado por data de referência do relatório."
                tone={runsByDate.length > 0 ? "default" : "soft"}
              >
                {!runsByDate.length ? (
                  ...
                  <div className="hile-section-stack">
                    {visibleRunsByDate.map(([dateLabel, runs]) => (
                      <div key={dateLabel}>
                    ...
                    {runsByDate.length > runsPerPage ? (
                      <PaginationControls
                        total={runsByDate.length}
                        safePage={safePage}
                        pages={pages}
                        onPrev={() => preserveWindowScroll(() => setHistoryPage((current) => Math.max(1, current - 1)))}
                        onNext={() => preserveWindowScroll(() => setHistoryPage((current) => Math.min(pages, current + 1)))}
                      />
                    ) : null}
                  </div>
                )}
