...
export async function GET(request: Request) {
  try {
    ...
    if (!runId) {
      return NextResponse.json({ error: "invalid_param", message: "Parâmetro run_id é obrigatório." }, { status: 400 });
    }

    const snapshot = await getRunSnapshot(runId);
    if (!snapshot) {
      return NextResponse.json({ error: "run_not_found", message: "Execução não encontrada." }, { status: 404 });
    }
